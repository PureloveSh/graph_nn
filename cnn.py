...
def conv(input_tenor, filter_tensor, output_tensor, stride, bias):
    ...
    for i in range(output_height):
        for j in range(output_width):
            output_tensor[i][j] = np.sum((get_patch(input_tenor, i, j, filter_width, filter_height, stride) * np.rot90(filter_tensor, 2)))+bias
...
class Filter(object):
    def __init__(self, filter_width=None, filter_height=None, filter_depth=None, filter_num=None, stride=None, downstream_node=None, upstream_node=None):
    ...
    def bp_gradient(self):
        ...
        for f in range(self.filter_num):
            ...
            for d in range(self.weights[f].shape[0]):
                ...
                conv(np.rot90(self.downstream_node.output[d], 2), self.upstream_node.delta[f], self.weights_grad[f][d], 1, 0)
...
class FullConnectionLayer:
    @staticmethod
    def forward(conn):
        return np.dot(conn.W, conn.downstream_node.output) + conn.b
...
class TensorConnection:
    ...
    def late_init(self, up_node_num, down_node_num):
    ...
    def update_weight(self, batch_size):
        ...
        '''
        self.bp_gradient()
        self.W -= float(self.upstream_node.learning_rate)*self.W_grad/batch_size
    ...
    def train(self):
        ...
        for i in range(10):
            for d in range(num_batch):
                l, a = self.train_one_sample(mnist.train.images[d].reshape((1, 28, 28)), mnist.train.labels[d].reshape((10, 1)))
                #loss = self.train_one_sample(mnist.train.images[d].reshape((784,1)), mnist.train.labels[d].reshape((10,1)))
                loss += l
                accuracy += a
                if (d+1) % print_epochs == 0:
                    print('Iter: {0} data:{1}  Train loss: {2}  Accuracy:{3}'.format(i+1, d+1, loss/print_epochs, accuracy/print_epochs))
                    loss, accuracy = 0, 0
        ...
        for d in range(10000):
            ...
            if (d+1) % test_epochs == 0:
                print('data:{0}  Test loss: {1}  Accuracy:{2}'.format(d + 1, loss / test_epochs, accuracy / test_epochs))
                loss, accuracy = 0, 0
