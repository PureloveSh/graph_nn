...
class TensorConnection(nn.Module):
    def __init__(self, upstream_node=None, downstream_node=None):
        super(TensorConnection, self).__init__()

        self.upstream_node = upstream_node
        self.downstream_node = downstream_node

    def late_init(self, up_node_num, down_node_num):
        self.layer1 = nn.Sequential(nn.Linear(up_node_num, down_node_num),
                                    nn.ReLU())
    ...
    def cal_output(self):
        ...
        for conn in self.downstream:
            if isinstance(conn, TensorConnection):
                ...
                # print('Filter connection:...', self.output)
            elif isinstance(conn, ConcatConnection):
                self.output += Concat.forward(conn)
                # print('Concat connection:...', self.output)
