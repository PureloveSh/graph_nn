...
class Capsule:
    def __init__(self,x,y,kind,CircleList,LineList):
        self.x=x
        self.y=y
        self.kind=kind
...
class CircleLine:
    def __init__(self,x,y,conn,kind,Linenum,startX,startY,endX,endY):
        ...
        self.MaskRate=1
class CapsuleLine:
    def __init__(self, x, y, kind, Linenum, startX, startY, endX, endY):
        self.x = x
        self.y = y
        self.kind = kind
        self.isSelect = 0
...
class MyCanvas:
    ...
    def drawMaskLine(self):
        ...
        # self.endY = endY
        # self.image = 0
        for line in self.CapsuleLineS:
            if line.MaskType == "随机连接":
                self.canvas.delete(line.num)
                num=self.canvas.create_line(line.startX,line.startY,line.endX,line.endY,
                                      fill=self.color,
                                      tags='line',
                                      width=3,
                                      arrow="last",
    ...
    def drawCrossType(self,kind):
        # x, y, kind, Linenum, startX, startY, endX, endY

        for start in self.capsuleList:
            if start.start == 1:
                for end in self.capsuleList:
                    if end.start == 0:
                        num=self.canvas.create_arc(start.x+30,start.y - 50 - self.cross*25,
                                               end.x-30,end.y+50+self.cross*25,
                                               extent = 180,style = ARC,width=3)
                        print("(",start.x+30,start.y,
                                               end.x - 30,end.y,")")

                        capsule=CapsuleLine(0, 0, kind, num, start.x + 30, start.y-50, end.x - 30, end.y)
                        self.CapsuleLineS.append(capsule)
                        if kind == "卷积连接":
                            image = self.canvas.create_image((start.x+end.x)/2,
                                                             end.y - 50 - self.cross*25,
                                                             image = self.im, tags="image")
                            capsule.image = image
                            pass
                        elif kind == "全连接":
                            image = self.canvas.create_image((start.x + end.x) / 2,
                                                             end.y - 50 - self.cross*25,
                                                             image=self.dense, tags="image")
                            capsule.image = image
                            pass
                        elif kind == "恒等连接":
                            # image = self.canvas.create_image((start.x + end.x) / 2,
                            ...
                            image = self.canvas.create_image((start.x + end.x) / 2,
                                                             ...
                                                             image=self.triangle, tags="image")
                            capsule.image = image
                            pass
                end.start = -1
            start.start = -1
    ...
    def drawReplaceCapsuleLine(self):
        for item in self.CapsuleLineS:
            if item.isSelect==1:
                item.isSelect = 0
                canvas1.canvas.delete(item.num)
                canvas1.canvas.delete(item.image)
                if item.kind == "卷积连接":
                    num,image=self.drawCNNLine(item.startX,item.startY,item.endX,item.endY)
                    item.num=num
                    item.image=image
                elif item.kind == "全连接":
                    denseNode=self.drawDenseLine(item.startX,item.startY,item.endX,item.endY)
                    item.num=denseNode.num
                    item.image=denseNode.image
                elif item.kind=="恒等连接":
                    num=self.drawEqualLine(item.startX,item.startY,item.endX,item.endY)
                    item.num=num
                elif item.kind=="变形连接":
                    num,image=self.drawReshapeLine(item.startX,item.startY,item.endX,item.endY)
                    item.num=num
                    item.image=image
                    pass
        pass
    def drawReshapeLine(self,startX, startY, endX, endY):
        num = self.canvas.create_line(startX, startY, endX, endY,
                                      fill=self.color,
                                      tags='line',
                                      width=3,
                                      arrow="last",
        ...
        x=(startX+endX)/2
        y=(startY+endY)/2
        image=self.canvas.create_image(x,y,image=self.triangle, tags="image")
        return num,image
        pass
    def drawcapsuleList(self):
        y = 350
        x = 135
        for i in range(len(self.capsuleList)):
            if self.capsuleList[i].kind == "一维数据胶囊":
                capsule=self.drawDataCapsule((i + 1) * x, y)
                self.capsuleList[i]=capsule
                pass
            elif self.capsuleList[i].kind == "一维RELU胶囊":
                capsule=self.drawONE_RELU((i + 1) * x, y)
                self.capsuleList[i] = capsule
                pass
            elif self.capsuleList[i].kind == "一维Softmax胶囊":
                capsule=self.ONE_SoftMax((i + 1) * x, y)
                self.capsuleList[i] = capsule
                pass
        for i in range(len(self.capsuleList) - 1):
            capsuleLine=self.drawDenseLine(self.capsuleList[i].x + 30, self.capsuleList[i].y,
                               self.capsuleList[i + 1].x - 30, self.capsuleList[i + 1].y)
            self.CapsuleLineS.append(capsuleLine)
        pass
    def deleteCapsule(self):
        for capsule in self.capsuleList:
            if capsule.isSelect==1:
                for i in capsule.CircleList:
                    canvas1.canvas.delete(i.num)
                    pass
                for j in capsule.LineList:
                    canvas1.canvas.delete(j.num)
                for k in capsule.TextList:
                    canvas1.canvas.delete(k)
                    pass
    def deleteCapsuleLine(self):
        deleteLineS=[]
        for capsuleLine in self.CapsuleLineS:
            if capsuleLine.isSelect==1:
                canvas1.canvas.delete(capsuleLine.num)
                canvas1.canvas.delete(capsuleLine.image)
                deleteLineS.append(capsuleLine)
        for i in deleteLineS:
            self.CapsuleLineS.remove(i)
    def drawAddLine(self,startCircle,endCircle):
        for i in startCircle:
            self.canvas.itemconfig(i.num, outline="black")
        for j in endCircle:
            self.canvas.itemconfig(j.num, outline="black")
        self.drawAllLine(startCircle,endCircle)
        pass
    def drawDataCapsule(self,x,y):
        num1 = self.canvas.create_rectangle(x - 30, y - 30, x + 30, y + 30,
                                           outline=self.color, tags='oval', width=3)
        circle0 = Circle(x, y, num1, self.CircleNum)
        CircleList = []

        num=self.canvas.create_oval(x - 7, y - 7, x + 7, y + 7,
                                outline=self.color, tags='oval', width=3)
        self.CircleNum+=1
        circle1=Circle(x,y,num,self.CircleNum)
        CircleList.append(circle1)

        num=self.canvas.create_oval(x - 7, y + 20 - 7, x + 7, y + 20 + 7,
                                outline=self.color, tags='oval', width=3)
        self.CircleNum += 1
        circle2 = Circle(x, y+20, num, self.CircleNum)
        CircleList.append(circle2)
        num=self.canvas.create_oval(x - 7, y - 20 - 7, x + 7, y - 20 + 7,
                                outline=self.color, tags='oval', width=3)
        self.CircleNum += 1
        circle3 = Circle(x, y-20, num, self.CircleNum)
        CircleList.append(circle3)
        CircleList.append(circle0)
        #
        capsule = Capsule(x, y, "一维数据胶囊", None, None)
        capsule.num=num1
        return capsule
        # self.capsuleList.append(capsule)
        pass
    def drawCapsuleNetwork(self,layers):
        y = 350
        x = 135
        lengh=len(layers)
        for i in range(lengh):
            if i == 0:
                capsule=self.drawDataCapsule((i+1)*x,y)
                capsule.nodeNum=layers[i]
                self.capsuleList.append(capsule)
                self.canvas.create_text(capsule.x, capsule.y + 50, text=capsule.nodeNum, tags="text")
                pass
            elif i < lengh - 1:
                capsule =self.drawONE_RELU((i+1)*x,y)
                capsule.nodeNum = layers[i]
        ...
        for i in range(len(self.capsuleList)-1):
            ...
            pass
        pass
    def add_circle(self,i,x,j,num=0,l=5):
        ...
        # print(num)
        if i == 0:
            num = self.canvas.create_rectangle(x - 30, y - 30, x + 30, y + 30,
                                          outline=self.color, tags='oval', width=3)
            circle = Circle(x, y, num, self.CircleNum)
            circle.Type = "数据神经元"
            pass
        elif i==l-1:
            num = self.canvas.create_oval(x - 30, y - 30, x + 30, y + 30,
                                          outline=self.color, tags='oval', width=3)
            self.canvas.create_line(x - 15 * math.sqrt(2), y + 15 * math.sqrt(2), x + 15 * math.sqrt(2),
                                    y - 15 * math.sqrt(2),
                                    fill=self.color,
                                    tags='line',
                                    width=3, )
        ...
        return circle
    def createCircle(self,i,x,j,num=0):
        if num!=0:
            if j<(num-1)/2:
                y = 350 - ( (num-1)/2 - j )*90
                pass
        ...
        circle = Circle(x, y, 0, self.CircleNum)
        print("self.CircleS[i][j].Type:",self.CircleS[i][j].Type)
        if self.CircleS[i][j].Type=="数据神经元":
            print("数据神经元")
            num = self.canvas.create_rectangle(x - 30, y - 30, x + 30, y + 30,
                                               outline=self.color, tags='oval', width=3)
            circle.num=num
            circle.Type = "数据神经元"
            pass
        elif self.CircleS[i][j].Type=="恒等神经元":
            print("恒等神经元")
            num = self.canvas.create_oval(x - 30, y - 30, x + 30, y + 30,
                                          outline=self.color, tags='oval', width=3)
            self.canvas.create_line(x - 15 * math.sqrt(2), y + 15 * math.sqrt(2), x + 15 * math.sqrt(2),
            ...
            circle.Type = "恒等神经元"
            pass
        elif self.CircleS[i][j].Type=="RELU神经元":
            print("RELU神经元")
            num = self.canvas.create_oval(x - 30, y - 30, x + 30, y + 30,
                                          outline=self.color, tags='oval', width=3)
            self.canvas.create_line(x - 30, y, x, y,
            ...
            circle.Type = "RELU神经元"
            pass
        elif self.CircleS[i][j].Type=="":

            pass
        self.CircleNum += 1
        return circle

    def drawDeleteLayerSCircle(self):
        for i in range(len(self.CircleS)):
            x = 115 + i * 150
            for j in range(len(self.CircleS[i])):
                circle=self.createCircle(i,x,j,len(self.CircleS[i]))
                self.CircleS[i][j] = circle

        for n in range(len(self.CircleS)-1):
            self.drawAllLine(self.CircleS[n],self.CircleS[n+1])
        pass
    def drawDeleteLineS(self):
        for i in range(len(self.CircleS)):
            x = 115 + i * 150
            for j in range(len(self.CircleS[i])):
                circle=self.createCircle(i,x,j,len(self.CircleS[i]))
                self.CircleS[i][j] = circle
        for n in range(len(self.CircleS)-1):
            self.drawAfterDeleteLineS(self.CircleS[n],self.CircleS[n+1])
        pass
    def drawAddCircle(self):
        for i in range(len(self.CircleS)):
            x = 115 + i * 150
            for j in range(len(self.CircleS[i])):
                circle=self.createCircle(i,x,j,len(self.CircleS[i]))
                self.CircleS[i][j] = circle
        self.CircleLineS.clear()
        for n in range(len(self.CircleS)-1):
            self.drawAllLine(self.CircleS[n],self.CircleS[n+1])
        pass
    def drawDeleteCircle(self):
        # for i in range(layers.__len__()):
        #     x=115+i*150
        #     circles = []
        #     for j in range(layers[i]):
        #         circle=self.add_circle(i,x,j,layers[i],len(layers))
    ...
    def drawCopyCircle(self,X,Y):
        ...
        for i in range(len(self.copyList)):
            if i == 0:
                num=self.canvas.create_oval(X - 30-195,Y - 30-15,X + 30-195,Y + 30-15, outline=self.color, tags='oval', width=3)
                preX = X-195
                preY = Y-15
                self.CircleNum+=1
                self.Circle.append(Circle(X-195, Y-15, num, self.CircleNum))
                ...
                num = self.canvas.create_oval(preX+copyX-30,copyY+preY-30,preX+copyX+30,preY+copyY+30,outline=self.color, tags='oval', width=3)

                self.CircleNum += 1
                self.Circle.append(Circle(preX + copyX, copyY + preY, num, self.CircleNum))
                # self.canvas.create_text(self.x, self.y, text=self.CircleNum,tags="text")
                node = Node(self.CircleNum)
                self.NodeCollection.add_node(node)
                preX=preX + copyX
                preY = preY + copyY
        for c in self.selectCircle:
            self.canvas.itemconfig(c.num, outline="black")
        self.selectCircle.clear()
        pass
    ...
    def drawCutCircle(self,X,Y):
        ...
        for i in range(len(self.cutList)):
            if i == 0:
                num = self.canvas.create_oval(X - 30 - 195, Y - 30 - 15, X + 30 - 195, Y + 30 - 15, outline=self.color,
                                        tags='oval', width=3)
                preX = X - 195
                preY = Y - 15
                self.CircleNum += 1
                ...
                copyX = self.cutList[i].x - self.cutList[i - 1].x
                copyY = self.cutList[i].y - self.cutList[i - 1].y
                print(self.cutList[i].x + copyX)
                print(self.cutList[i].y + copyY)
                num=self.canvas.create_oval(preX + copyX - 30, copyY + preY - 30, preX + copyX + 30, preY + copyY + 30,
                                        outline=self.color, tags='oval', width=3)
                self.CircleNum += 1
                self.Circle.append(Circle(preX + copyX, copyY + preY, num, self.CircleNum))
                # self.canvas.create_text(self.x, self.y, text=self.CircleNum,tags="text")
    ...
    def DeleteCircle(self):
        for item in  self.deleteList:
            ...
            x=item.x
            y=item.y
            for line in self.CircleLineS:
                if math.sqrt((line.startX-x)*(line.startX-x)+(line.startY-y)*(line.startY-y))<=30:
                    self.canvas.delete(line.num)
        pass
    def replaceDenseinit(self,replaceType):
        ...
        #         circles.append(circle)
        #     self.CircleS.append(circles)
        # "RELU神经元", "Sigmoid神经元", "Tanh神经元", "恒等神经元"
        # , "Softmax神经元" "ReLU神经元","PReLU神经元"
        # for i in range(len(self.CircleS)):
        #     for j in range(len(self.CircleS[i])):
        #         if self.CircleS[i][j].isSelect==1:
        #             if replaceType=="RELU神经元":
        #                 x=self.CircleS[i][j].x
        #                 y=self.CircleS[i][j].y
        #                 self.canvas.create_line(x - 30, y, x, y,
        #                                         fill=self.color,
        #                                         tags='line',
        #                                         width=3, )
        ...
        #                                         width=3, )
        #                 pass
        #             elif replaceType=="Sigmoid神经元":
        #
        #                 pass
        #             elif replaceType=="Tanh神经元":
        #
        #                 pass
        #             elif replaceType=="恒等神经元":
        #                 x=self.CircleS[i][j].x
        #                 y=self.CircleS[i][j].y
        #                 self.canvas.create_line(x - 15 * math.sqrt(2), y + 15 * math.sqrt(2), x + 15 * math.sqrt(2),
        #                                         y - 15 * math.sqrt(2),
        ...
        #                                         width=3, )
        #                 pass
        #             elif replaceType=="Softmax神经元":
        #
        #                 pass
        #             elif replaceType=="RELU神经元":
        #
        #                 pass
        #             elif replaceType=="PReLU神经元":
        #
        #                 pass
        for i in range(len(self.CircleS)):
            x = 115 + i * 150
    ...
    def DenseInit(self,layers):
        copyLayers = []
        for k in range(layers.__len__()):
            if layers[k]!=0:
                copyLayers.append(layers[k])
        layers = copyLayers
        for i in range(layers.__len__()):
            x=115+i*150
            circles = []
        ...
        for n in range(len(self.CircleS)):
            for j in range(n+1,len(self.CircleS)):
    ...
    def drawDenseLine(self,startX, startY, endX, endY):
        ...
        num=self.canvas.create_image(x,y,image=self.dense, tags="image")
        capsuleline=CapsuleLine(x,y,"全连接",Linenum,startX,startY,endX,endY)
        capsuleline.image=num
        return capsuleline
        pass
    ...
    def drawCNNLine(self,startX,startY,endX,endY):
        ...
        num=self.canvas.create_line(startX, startY, endX, endY,
                                ...
                                arrow="last",
                                arrowshape='8 10 3')
        image=self.canvas.create_image(x, y, image=self.im,tags="image")
        return num,image
        pass
    def clearAll(self):
        self.copyList.clear()
    ...
    def drawAllLine(self,startCircle,endCircle):
        for c3 in startCircle:
            for c4 in endCircle:
                ...
                kind = ""
                conn = ""
                line=CircleLine(x,y,conn,kind,num,startX, startY, endX, endY)
                self.Linenum += 1
                self.CircleLineS.append(line)
                # print("-------------------------num:")
                # if a < 0 and b < 0:
                ...
                #                             arrow="last",
                #                             arrowshape='8 10 3')
    def drawAfterDeleteLineS(self,startCircle,endCircle):

        for item in self.CircleLineS:
            if item.isSelect==0:

                num = self.canvas.create_line(item.startX, item.startY,
                                                item.endX, item.endY,
                                                    fill=self.color,
                                                    tags='line',
                                                    width=3,
    ...
    def Press(self,event):
        ...
        if self.state == 'StartCircle':
            ...
            for c in self.capsuleList:
                if math.sqrt(math.fabs(event.x - c.x) * math.fabs(event.x - c.x) + math.fabs(
                                event.y - c.y) * math.fabs(
                            event.y - c.y)) < (30):
                    self.canvas.itemconfig(c.num, outline="red")
                    print("---->被选中的胶囊")
                    c.start = 1
        ...
        if self.state == 'SelectCircle':
            ...
            for c in self.CircleLineS:
                ...
                print("c.x:", c.x)
                print("c.y:", c.y)
                if math.sqrt(math.fabs(event.x-c.x)*math.fabs(event.x-c.x)+math.fabs(event.y-c.y)*math.fabs(event.y-c.y))<(7):
                    if c.isSelect==1:
                        continue
                    else:
                        c.isSelect = 1
            ...
            for c3 in startCircle:
                for c4 in endCircle:
                    ...
                    num=self.canvas.create_line(c3.x + 30, c3.y, c4.x - 30, c4.y,
                                                                        ...
                                                                        arrowshape='8 10 3')
                    print("num:",num)
                    circleLine=CircleLine((c3.x+c4.x)/2,(c3.y+c4.y)/2,None,None,num,c3.x + 30,c3.y,
                                          c4.x - 30,c4.y)
                    self.CircleLineS.append(circleLine)
                    # if a<0 and b<0:
                    #     self.canvas.create_line(c3.x+20, c3.y+20, c4.x-20, c4.y-20,
                    #                             fill=self.color,
                    #                             tags='line',
    ...
    def ONE_SoftMax(self,x,y):
        ...
        num1 = self.canvas.create_oval(x - 30, y - 30, x + 30, y + 30,
                                      outline=self.color, tags='oval', width=3)
        circle1=Circle(x,y,num1,self.CircleNum)

        num=self.canvas.create_oval(x - 7, y - 7, x + 7, y + 7,
                                outline=self.color, tags='oval', width=3)
        circle2 = Circle(x, y, num, self.CircleNum)

        num=self.canvas.create_oval(x - 7, y + 18 - 7, x + 7, y + 18 + 7,
                                outline=self.color, tags='oval', width=3)
        circle3 = Circle(x,y+18, num, self.CircleNum)

        num=self.canvas.create_oval(x - 7, y - 18 - 7, x + 7, y - 18 + 7,
                                outline=self.color, tags='oval', width=3)
        circle4 = Circle(x, y - 18, num, self.CircleNum)
        CircleList.append(circle1)
        CircleList.append(circle2)
        CircleList.append(circle3)
        CircleList.append(circle4)
        textList = []
        num=self.canvas.create_text(x - 15,y,text="S",tags="text")
        textList.append(num)
        num=self.canvas.create_text(x + 15, y, text="M",tags="text")
        textList.append(num)
        capsule = Capsule(x,y,"一维Softmax胶囊",CircleList,[])
        capsule.TextList = textList
        self.CircleNum += 1
        capsule.num=num1
        return capsule
        # self.capsuleList.append(capsule)
    ...
    def drawONE_RELU(self,x,y):
        ...
        num=self.canvas.create_oval(x - 7, y + 18 - 7, x + 7, y + 18 + 7,
                                outline=self.color, tags='oval', width=3)
        self.CircleNum += 1
        circle3 = Circle(x, y, num, self.CircleNum)

        num=self.canvas.create_oval(x - 7, y - 18 - 7, x + 7, y - 18 + 7,
                                outline=self.color, tags='oval', width=3)
        self.CircleNum += 1
        circle4 = Circle(x, y, num, self.CircleNum)
        CircleList.append(circle1)
        CircleList.append(circle2)
        CircleList.append(circle3)
        CircleList.append(circle4)
        line1 = self.canvas.create_line(x - 30, y, x, y,
                                fill=self.color,
                                tags='line',
                                width=3,
                                arrowshape='8 10 3')

        line2 = self.canvas.create_line(x, y, x + 15 * math.sqrt(2), y - 15 * math.sqrt(2),
                                fill=self.color,
                                tags='line',
                                width=3,
                                arrowshape='8 10 3')
        ...
        capsule = Capsule(x, y, "一维RELU胶囊", CircleList, lineList)
        capsule.num = num1
        # self.capsuleList.append(capsule)
        return capsule
        # self.CircleNum += 1
...
def anotherWindow():
    ...
    def getListlayers():
        ...
        for item in entryList:
            print(layers.append(int(item.get())))
        ...
        if content.isdigit() or (content == ""):
            if content.isdigit():
                for i in range(int(content)):
                    # print("e00000")
                    if i==0:
                        l=Label(root1,text = "输入层")
                        l.place(x=80,y=30)

                        # l.pack(side=LEFT)
                        # print(Label(root1,text = "输入层").grid(row=i+1,column=2))
                        labelList.append(l)
                        e0 = Entry(root1)
                        e0.place(x=130,y=30)
                        e0.insert(END,"3")
                        # e0.pack(side=LEFT)
                        entryList.append(e0)
                    elif i<int(content)-1:
                        l = Label(root1,text = "隐藏层")
                        l.place(x=80,y=(i+1)*30)
                        # l.pack()
                        labelList.append(l)
                        e0 = Entry(root1)
                        e0.place(x=130,y=(i+1)*30)
                        e0.insert(END, "3")
                        # e0.pack()
                        entryList.append(e0)
                    elif i==int(content)-1:
                        l = Label(root1,text = "输出层")
                        l.place(x=80,y=(i+1)*30)
                        # l.pack()
                        labelList.append(l)
                        e0 = Entry(root1)
    ...
    v = StringVar()
    testCMD = root1.register(onChange)  # 需要将函数包装一下，必要的
    e0 = Entry(root1,textvariable=v,validate='key',validatecommand=(testCMD,"%P"),width=10)
    e0.insert(1,"3")
    e0.place(x=80,y=0)
    onChange("3")
    # e0.pack()
    ...
    button = Button(root1, text="确定", height=1,command = getListlayers)
...
commonframe=Frame(root,width=150,height=30,bg='red')
...
#     global MLPClick
#     MLPClick=False
#     global CapsuleLayers
#
#     anotherWindowFive()
#     print("CapsuleLayers:", CapsuleLayers)
#     pass
CapsuleLayers=0
CapsuleClick=False
def anotherWindowFive():
    canvas1.clearAll()
    canvas1.capsuleList.clear()
    canvas1.state = ""
    global MLPClick
    MLPClick = False
    global CapsuleLayers
    ...
    def getLayerS():
        ...
        CapsuleLayers=[]
        for item in entryList:
            CapsuleLayers.append(int(item.get()))
        canvas1.drawCapsuleNetwork(CapsuleLayers)
        print("CapsuleLayers:",CapsuleLayers)
        root1.destroy()
        root1.quit()
        ...
        if content.isdigit() or (content == ""):
            if content.isdigit():
                for i in range(int(content)):
                    # print("e00000")
                    if i==0:
                        l=Label(root1,text = "第"+str(i+0)+"层")
                        l.place(x=80,y=30)
                        # l.pack(side=LEFT)
                        # print(Label(root1,text = "输入层").grid(row=i+1,column=2))
                        labelList.append(l)
    ...
    # e0.pack()
    # e0 = Entry(root1,width=10)
    # e0.place(x=80, y=0)
    # def getLayerS():
    #     print("胶囊网络层数:",e0.get())
    #     CapsuleLayers = e0.get()
    #     print("CapsuleLayers:",CapsuleLayers)
    #     canvas1.drawCapsuleNetwork(int(CapsuleLayers))
    #     root1.destroy()
    #     root1.quit()

    button = Button(root1, text="确定", height=1, command=getLayerS)
    button.place(x=150, y=0)
    root1.mainloop()
    pass
CapsuleButton=Button(commonframe,text="胶囊网络",command=anotherWindowFive)
CapsuleButton.place(x=62,y=0)
...
def delete():
    canvas1.deleteList.clear()
    canvas1.deleteList.extend(canvas1.selectCircle)
    print("deleteList_len",len(canvas1.deleteList))
    canvas1.DeleteCircle()
    pass

def deleteCircle():
    canvas1.canvas.delete("line")
    canvas1.canvas.delete("oval")
    for i in canvas1.CircleS:
        for j in i:
            if j.isSelect == 1:
                i.remove(j)
    # canvas1.layers = []
    for i in canvas1.CircleS:
        ...
        pass
    # canvas1.CircleS = []
    # print(canvas1.layers)
    # canvas1.DenseInit(canvas1.layers)
    canvas1.drawDeleteCircle()
    pass
nodeType="恒等神经元"
nodeNum=1
def anotherWindowTwo():
    ...
    comvalue = StringVar()
    comboxlist = ttk.Combobox(root1,textvariable=comvalue)
    comboxlist["values"] = ["RELU神经元","Sigmoid神经元","Tanh神经元","恒等神经元"
                                ,"Softmax神经元","数据神经元","PReLU神经元"]
    comboxlist.current(0)
    Listlayers.append(comboxlist)
    comboxlist.pack()
    e0 = Entry(root1,width=10)
...
def anotherWindowThree():
    ...
    def getListlayers():
        global replacenodeType
        replacenodeType=comboxlist.get()
        root1.destroy()
        root1.quit()
        pass
    button = Button(frame, text="确定", height=1,command = getListlayers)
...
def addCircle():
    ...
    n = 0
    for i in range(len(canvas1.CircleS)):
        for j in canvas1.CircleS[i]:
            if j.isSelect == 1:
                n=i
                break
    # print("canvas1.layers:", canvas1.layers)
    anotherWindowTwo()

    print("comvalue:", nodeType)
    print("e0.get:", nodeNum)
    # print("n:",n)
    #
    # for i in range(len(canvas1.layers)):
    #     print("i之后:", i)
    #     if i==n:
    #         canvas1.layers[i]=canvas1.layers[i]+int(nodeNum)
    #     print("i之2前:",i)
    # print("layers:",canvas1.layers)
    # canvas1.CircleS = []
    circle=[]
    for j in range(int(nodeNum)):
        ...
        C.Type=nodeType
        circle.append(C)
    canvas1.CircleS[n].extend(circle)
    canvas1.canvas.delete("line")
    canvas1.canvas.delete("oval")
    for i in canvas1.CircleS:
        print("删除之后:",len(i))
    ...
    canvas1.drawAddCircle()

    # canvas1.DenseInit(canvas1.layers)
    pass
def replaceCircle():
    anotherWindowThree()
    print("replacenodeType",replacenodeType)
    # replaceList=[]
    canvas1.canvas.delete("line")
    canvas1.canvas.delete("oval")
    for i in range(len(canvas1.CircleS)):
        circles=[]
        for j in range(len(canvas1.CircleS[i])):
            if canvas1.CircleS[i][j].isSelect==1:
                canvas1.CircleS[i][j].Type=replacenodeType

    canvas1.replaceDenseinit(replacenodeType)
    pass
Layernum=1
def anotherWindowFour():
    root1 = Tk()
    e0 = Entry(root1, width=10)
    e0.pack()
...
def deleteLayers():
    ...
    dy=[]
    for k in n:
        dy.append(canvas1.CircleS[k])
    for i in dy:
        canvas1.CircleS.remove(i)
    canvas1.drawDeleteLayerSCircle()
    pass
...
def addLayers():
    ...
    for i in range(int(Layernum)):
        ...
        circles.append(circle)
    AddCircles=[]
    CircleS1=canvas1.CircleS[0:n]
    CircleS2=canvas1.CircleS[n:len(canvas1.CircleS)]
    AddCircles.extend(CircleS1)
    AddCircles.extend(circles)
    AddCircles.extend(CircleS2)
    canvas1.CircleS = AddCircles
    canvas1.canvas.delete("line")
    canvas1.canvas.delete("oval")
    canvas1.drawAddCircle()
    pass
def addConection():
    ...
    for i in canvas1.CircleS:
        for j in i:
            if j.start==1:
                startCircle.append(j)
            elif j.start==0:
                endCircle.append(j)
    canvas1.drawAddLine(startCircle,endCircle)
    pass
def deleteConection():
    canvas1.canvas.delete("line")
    canvas1.canvas.delete("oval")
    canvas1.drawDeleteLineS()
    pass

menu = Menu(canvas1.canvas, tearoff=0)
menu.add_command(label="删除神经元",command=deleteCircle)
menu.add_separator()
menu.add_command(label="添加神经元",command=addCircle)
menu.add_separator()
menu.add_command(label="替换神经元",command=replaceCircle)
menu.add_separator()
menu.add_command(label="删除层",command = deleteLayers)
menu.add_separator()
menu.add_command(label="添加层",command = addLayers)
menu.add_separator()
menu.add_command(label="添加连接",command = addConection)
menu.add_separator()
menu.add_command(label="删除连接",command = deleteConection)
menu.add_separator()

def deleteCapsule():
    canvas1.canvas.delete("image")
    canvas1.canvas.delete("line")
    canvas1.canvas.delete("oval")
    canvas1.canvas.delete("text")
    # for item in canvas1.capsuleList:
    #     print(item.isSelect)
    deleteList=[]
    for item in canvas1.capsuleList:
        if item.isSelect==1:
    ...
    for i in deleteList:
        canvas1.capsuleList.remove(i)
    canvas1.drawcapsuleList()
    pass

CapsulenodeType = "一维RELU胶囊"
CapsulenodeNum = 0
def anotherWindowSix():
    root1 = Tk()
    Listlayers = []
    root1.geometry('300x400')
    frame = Frame(root1, width=300, height=400, bg='pink')
    ...
    comvalue = StringVar()
    comboxlist = ttk.Combobox(root1,textvariable=comvalue)
    comboxlist["values"] = ["一维RELU胶囊","一维Sigmoid胶囊","一维Tanh胶囊",
    "一维恒等胶囊","一维Softmax胶囊","二维数据胶囊","二维RELU胶囊","二维Sigmoid胶囊",
    "二维Tanh胶囊","二维恒等胶囊","二维最大下采样胶囊","二维平均下采样胶囊","二维Pading胶囊","二维拼接胶囊"]
    comboxlist.current(0)
    Listlayers.append(comboxlist)
    comboxlist.pack()
    e0 = Entry(root1,width=10)
    e0.pack()
    def getListlayers():
        global CapsulenodeType
        global CapsulenodeNum
        CapsulenodeType=comboxlist.get()
        CapsulenodeNum=e0.get()
        root1.destroy()
        root1.quit()
        pass
    button = Button(frame, text="确定", height=1,command=getListlayers)
...
def anotherWindowSeven():
    ...
    comboxlist.pack()
    def getListlayers():
        global CapsuleReplaceType
        CapsuleReplaceType=comboxlist.get()
        root1.destroy()
        root1.quit()
        pass
    button = Button(frame, text="确定", height=1,command=getListlayers)
...
def addCapsule():
    anotherWindowSix()
    n = 0
    for i in range(len(canvas1.capsuleList)):
        if canvas1.capsuleList[i].isSelect==1:
            canvas1.capsuleList[i].isSelect == 0
            n=i
            break
    dy = []
    print("CapsulenodeNum:",CapsulenodeNum)
    for i in range(int(CapsulenodeNum)):
        casule=Capsule(0,0,CapsulenodeType,[],[])
        dy.append(casule)
        print("添加胶囊")

    cap = []
    cap.extend(canvas1.capsuleList[0:n])
    cap.extend(dy)
    cap.extend(canvas1.capsuleList[n:len(canvas1.capsuleList)])

    canvas1.capsuleList.clear()
    canvas1.capsuleList.extend(cap)
    for i in canvas1.capsuleList:
        print("addCapsule:",i.kind)
    canvas1.canvas.delete("image")
    canvas1.canvas.delete("line")
    canvas1.canvas.delete("oval")
    canvas1.canvas.delete("text")
...
def deleteCapsuleCon():
    canvas1.deleteCapsuleLine()
    pass
CapsuleLineReplaceType =""
def anotherWindowEight():
    root1 = Tk()
    Listlayers = []
    root1.geometry('300x400')
    frame = Frame(root1, width=300, height=400, bg='pink')
    comvalue = StringVar()
    comboxlist = ttk.Combobox(root1,textvariable=comvalue)
    comboxlist["values"] = ["卷积连接","恒等连接","变形连接","全连接","部分连接"]
    comboxlist.current(0)
    Listlayers.append(comboxlist)
    comboxlist.pack()
    def getListlayers():
        global CapsuleLineReplaceType
        CapsuleLineReplaceType = comboxlist.get()
        root1.destroy()
        root1.quit()
        pass
    button = Button(frame, text="确定", height=1,command=getListlayers)
...
def replaceCapsuleCon():
    anotherWindowEight()
    for i in canvas1.CapsuleLineS:
        if i.isSelect==1:
            i.kind=CapsuleLineReplaceType
    for i in canvas1.CapsuleLineS:
        print(i.kind)
    canvas1.drawReplaceCapsuleLine()
    pass
ConnectionType = ""
ConnectionRate = 1
def anotherWindowNine():
    root1 = Tk()
    root1.geometry('300x400')
    frame = Frame(root1, width=300, height=400, bg='pink')
    comvalue = StringVar()
    comboxlist = ttk.Combobox(root1,textvariable=comvalue)
    comboxlist["values"] = ["全连接","随机连接","自定义连接"]
    comboxlist.current(0)
    comboxlist.pack()
    e0 = Entry(root1,width=10)
    e0.pack()
    def getListlayers():
        ...
        global ConnectionRate
        ConnectionType = comboxlist.get()
        ConnectionRate = e0.get()
...
def addMask():
    anotherWindowNine()
    for item in canvas1.CapsuleLineS:
        if item.isSelect == 1:
            item.MaskType = ConnectionType
            item.MaskRate = float(ConnectionRate)

    canvas1.drawMaskLine()

    pass
CapsuleLineCrossType =""
def anotherWindowTen():
    root1 = Tk()
    Listlayers = []
    root1.geometry('300x400')
    frame = Frame(root1, width=300, height=400, bg='pink')
    ...
    comboxlist.pack()
    def getListlayers():
        global CapsuleLineCrossType
        CapsuleLineCrossType = comboxlist.get()
        root1.destroy()
        root1.quit()
        pass
    button = Button(frame, text="确定", height=1,command=getListlayers)
...
def addCrossConn():
    anotherWindowTen()
    canvas1.drawCrossType(CapsuleLineCrossType)
    pass

menu1 = Menu(canvas1.canvas, tearoff=0)
menu1.add_command(label="删除胶囊",command=deleteCapsule)
menu1.add_separator()
menu1.add_command(label="添加胶囊",command=addCapsule)
menu1.add_separator()
menu1.add_command(label="替换胶囊",command=replaceCapsule)
menu1.add_separator()
menu1.add_command(label="删除连接",command=deleteCapsuleCon)
menu1.add_separator()
menu1.add_command(label="替换连接",command=replaceCapsuleCon)
menu1.add_separator()
menu1.add_command(label="添加Mask",command=addMask)
menu1.add_separator()
menu1.add_command(label="添加跨连",command=addCrossConn)
menu1.add_separator()
